...
# MCP servers reachable via --mcp-servers-config. These are spawned as child
# processes, so their runtime has to exist in this image. Install the tools at
# build time rather than resolving them through `uvx` on every start: tool
# discovery at startup is capped at 10s per server, which a cold fetch blows past.
