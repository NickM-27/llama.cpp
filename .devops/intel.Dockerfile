...
RUN if [ "${GGML_SYCL_F16}" = "ON" ]; then \
    ...
    cmake -B build -DGGML_NATIVE=OFF -DGGML_SYCL=ON -DCMAKE_C_COMPILER=icx -DCMAKE_CXX_COMPILER=icpx -DGGML_BACKEND_DL=ON -DGGML_CPU_ALL_VARIANTS=ON -DGGML_SYCL_ESIMD=ON -DLLAMA_BUILD_TESTS=OFF ${OPT_SYCL_F16} && \
