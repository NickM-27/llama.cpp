...
// Fused UNARY + MUL. Unlike the gated ops above, which split one tensor into halves, `x`
// and `g` are separate tensors of the same shape; `o0`/`o1` are their row strides in
// elements, so a GLU-style half-view (what the Conformer audio encoders build for their
// sigmoid gate) is handled without a repack. `dst` is contiguous and indexed flat.
// Activation and product are evaluated in f32, as the CPU and CUDA references do.
template<typename T, typename F>
...
static void unary_mul_sycl(const T * x, const T * g, T * dst, const int64_t k, const int64_t n, const int64_t o0, const int64_t o1, queue_ptr main_stream, F op) {
    const size_t            num_blocks = ceil_div((size_t) k, (size_t) SYCL_SILU_BLOCK_SIZE);
    const sycl::nd_range<1> range(num_blocks * sycl::range<1>(SYCL_SILU_BLOCK_SIZE), sycl::range<1>(SYCL_SILU_BLOCK_SIZE));

    // o0 == o1 == n makes (i/n)*o0 + (i%n) == i, so the strided kernel degenerates to the
    // flat one; take it directly, as ggml_sycl_op_unary does for the contiguous case.
    if (o0 == n && o1 == n) {
...
// dst = op(unary_node->src[0]) * other, written straight to the MUL output. Saves the
// standalone unary launch and the round trip of its result through global memory.
// Preconditions are established by ggml_sycl_can_fuse(); they are re-asserted here.
void ggml_sycl_op_unary_mul_fused(ggml_backend_sycl_context & ctx, ggml_tensor * unary_node, ggml_tensor * mul_node) {
    ...
    // g is picked by elimination, so MUL(unary, unary) would alias it onto the output this
    // kernel never writes; ggml_can_fuse()'s single-use rule already rules that graph out
    ...
    // instantiates the kernel for each (type, unary op) pair without spelling out the cross product
