...
void ggml_sycl_flash_attn_ext_onednn(ggml_backend_sycl_context & ctx, ggml_tensor * dst) try {
    ...
    // serializes with the staging kernels before it (including the K/V dequant staging into
    // Kf_pool/Vf_pool) and the permute/pool reuse after it. The garbage output formerly blamed
    // on the missing sync here was the scale use-after-return fixed above. Keep the conservative
    // wait for multi-GPU, where other devices' streams can race the pool:
